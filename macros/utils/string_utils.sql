...
{%- macro hash(field) -%}
  {{ return(adapter.dispatch('hash', 'thesis_dbt') (field)) }}
{%- endmacro -%}

{%- macro default__hash(field) -%}
    md5(cast({{field}} as {{thesis_dbt.type_string()}}))
{%- endmacro -%}

{%- macro bigquery__hash(field) -%}
    to_hex({{thesis_dbt.default__hash(field)}})
{%- endmacro -%}



{%- macro concat(fields) -%}
  {{ return(adapter.dispatch('concat', 'thesis_dbt')(fields)) }}
{%- endmacro -%}
...
{%- macro surrogate_key(field_list) -%}
    ...
    {{ return(adapter.dispatch('surrogate_key', 'thesis_dbt')(field_list, *varargs)) }}
{%- endmacro -%}
...
{%- for field in field_list_xf -%}
    ...
    {%- set _ = fields.append(
        "coalesce(cast(" ~ field ~ " as " ~ thesis_dbt.type_string() ~ "), '')"
    ) -%}
